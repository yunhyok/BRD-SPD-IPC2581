...
For the desktop workflow, choose **노트북 (작업 요청)** before using Native SKILL. The Native SKILL tab is disabled in **워크스테이션 (Allegro 실행)** mode; stop an active Agent and switch to the laptop role first. The **Help / 도움말** menu can open any topic, while **F1** opens the guide for the current role in the built-in viewer. Chrome, Edge, WebView2, and an internet installation are not required. The viewer supports its table of contents, document links, and smaller/larger text. Load the SPD first; target lists, component options and generation remain disabled until loading succeeds. Browsing loads automatically, while a typed or restored path requires **SPD 불러오기**. Layer and net dialogs support select all, clear and Ctrl/Shift multi-selection.
...
The SKILL program opens `base.brd`, validates every selected layer and net, starts a database transaction, replaces the selected plane shapes, updates dynamic shapes globally, and saves a separate `result.brd`. Because the command line above already passes `base.brd` to Allegro, the script first reads `axlCurrentDesign()`: an already-open `base.brd` is reused and logged, `axlOpenDesignForBatch` is called only when no design is open, and any other open design fails the run instead of risking a save prompt under `-nograph`. That guard is unverified on a real installation. Coordinates stay in millimetres in the generated source and are converted to the active board units with `axlMKSConvert`; the board unit setting is not changed.
...
- Positive polygon and circle records become solid static `ETCH/<layer>` shapes. A `.Shape` section that several `Patch...` records map onto different layers is emitted once per mapped layer, and each layer/net pair appears in `manifest.json`.
- Negative records become permanent polygon or circular voids in every containing positive shape on the same net, because an SPD negative removes copper from every same-net shape it overlaps. A negative without a net attaches to any containing selected positive shape. Multiple parents are counted as `NATIVE_VOID_MULTIPLE_PARENTS`, and a parent that occurs after its void in the file as `NATIVE_VOID_ORDER_FALLBACK`. A void whose own net is not part of the selection and that no selected shape of that net contains is not emitted and is counted as `NATIVE_VOID_OUTSIDE_SELECTION`; a negative on one net never removes copper from another net, so neighbouring foreign copper does not make it ambiguous. A void that is not contained in one shape but spans the seam between two overlapping same-net shapes is refused rather than split, because a partially applied void is unverified in Allegro: split the void or merge the shapes in the SPD. A netless void contained by overlapping shapes of different nets is refused as well, since different nets sharing copper on one layer is a short. Ambiguous (boundary-crossing), unmatched, or invalid void geometry stops bundle generation before an executable bundle is published.
- For each selected layer/net pair, existing dynamic `BOUNDARY/<layer>` shapes and existing static `ETCH/<layer>` shapes are deleted before replacement. An ETCH shape flagged `shapeBoundary`, `shapeIsBoundary`, or `shapeAuto` is skipped as auto-generated fill and the skip count is written to `execution.log`; the exact semantics of those three attributes are unverified, so that count is how a wrong assumption becomes visible.
- Nets and components are resolved with `axlDBFindByName`, and only fall back to `axlSelectByName` inside an explicit `axlClearSelSet` / `axlSetFindFilter` / `axlClearSelSet` sequence, because `axlSelectByName` writes the current selection set and obeys the user's Find Filter. `execution.log` records which path resolved each object.
- With `update_components=True`, an existing placed refdes is moved and rotated with `axlTransformObject`. The SPD placement layer must be the first or last conductor layer, and it must match the base symbol side. A missing, unplaced, or side-mismatched component fails the native transaction. Because the order in which Allegro applies `?move` and `?angle` could not be confirmed, the update is two calls: a rotation about the symbol's current origin, which leaves the origin where it is, and then a move computed from the symbol position read back after that rotation. Each call is skipped when it would be a no-op. The rotation sense for mirrored bottom-side symbols remains an unverified assumption to check on a real board.
...
The emitted APIs and behavior were checked against the installed Allegro 24.1 SKILL reference under `share\pcb\examples\skill\DOC\FUNCS`, including `axlOpenDesignForBatch`, `axlDBGetShapes`, `axlDeleteObject`, `axlDBCreateOpenShape`, `axlDBCreateVoid`, `axlDBCreateVoidCircle`, `axlDBCreateCloseShape`, `axlDBTransactionStart`, `axlTransformObject`, `axlMKSConvert`, and `axlSaveDesign`. Every emitted polygon path repeats its first point as its last point, because the reference examples close a boundary explicitly and a closing segment is not documented as implicit. `axlDBFindByName`, `axlSetFindFilter`, `axlGetSelSet`, `axlClearSelSet`, `axlCurrentDesign` and the string helpers used by the already-open guard come from public SKILL examples instead, and are unverified on the installation. Native execution still requires a licensed Allegro 24.1 installation and a real board review.
